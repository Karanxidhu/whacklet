...
export async function createWallet(name: string, chatId: string) {
  try {
...
export async function getBalance(name: string, chatId: string) {
  try {
...
export async function transfer(from: Wallet, to: string, amount: string) {
  try {
...
export async function createNFT(
  ...
  try {
    ...
    const defaultWallet = wallet.wallets.find(
      (w) => w.id === wallet.defaultWallet
    );
    if (!defaultWallet)
      return { success: false, error: "Default wallet not found" };

    const payer = Keypair.fromSecretKey(
      bs58.decode(aesDecrypt(defaultWallet.privateKey, SECRET))
    );
...
export async function transferNFT(
  ...
  try {
    const payer = Keypair.fromSecretKey(
      bs58.decode(aesDecrypt(from.privateKey, SECRET))
    );
...
export async function getTokenAccounts(walletPublicKey: PublicKey) {
